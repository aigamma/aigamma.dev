...
export default function SlotE() {
  ...
  // SVI Jump-Wing fit deferred to requestIdleCallback so the card paints
  // its chrome before the multi-start LM fit runs.
  ...
  useEffect(() => {
    ...
    const layout = plotly2DChartLayout({
      title: {
        ...plotlyTitle(
          mobile
            ? 'σ(k) · SVI-JW Parameters<br>on the Smile'
            : 'σ(k) · SVI-JW Parameters on the Smile'
        ),
