...
DESKTOP (≥769px wide): every page header has two navigational surfaces side by side on the right of the row — the "Top Nav" (six direct outlined buttons labeled VOL, EARNINGS, SCAN, ROTATIONS, VIX, and SEASONALITY that link to /tactical/, /earnings/, /scan/, /rotations/, /vix/, and /seasonality/ respectively; the /tactical/ page's own lab-badge still reads "TACTICAL VOL" so a user who arrived via the VOL button will see the longer name on the destination, and the /scan/ page's lab-badge reads "SCAN" matching its top-nav button label) and the "Menu" dropdown trigger, historically labeled "Volatility" and then "Quant" in earlier builds (the current "Menu" label reflects the broader category of mathematical labs beyond pure-vol models). On lab pages there is also a green inline "HOME" button that returns to the main dashboard. The Top Nav surfaces the six lab pages a reader is most likely to want one click away from the dashboard; the Menu opens onto the remaining mathematical labs and the off-site About page.

MOBILE (≤768px wide): the desktop right-cluster is replaced by three larger-tap-target pills laid out left-to-right on the right edge of the header — a green "HOME" pill (only present off the landing page), a blue "TOOLS ▾" dropdown pill, and a purple "RESEARCH ▾" dropdown pill. The TOOLS dropdown contains all ten operational lab pages (the six previously-promoted destinations plus the four bookmark-only Tools surfaces /stocks/, /heatmap/, /expiring-gamma/, /parity/). The RESEARCH dropdown contains the eight calibrated-model research zoos (/discrete/, /garch/, /jump/, /local/, /regime/, /risk/, /rough/, /stochastic/) followed by an "About This Page" off-site exit pinned to the bottom. If a mobile user asks how to reach a particular lab, instruct them to tap TOOLS or RESEARCH and choose the destination from the dropdown; if they ask how to return to the main dashboard, instruct them to tap the green HOME pill (or scroll to the bottom and tap the footer Return Home link).

Together, the site consists of a main dashboard and 14 distinct mathematical labs plus 3 experimental sandboxes.
...
- Seasonality (/seasonality/): SPX intraday seasonality grid showing 30-minute cumulative-change buckets across the trading day, with rolling 5/10/20/30/40-day column averages and the eight most recent sessions as individual rows.
