...
import { daysToExpiration, filterPickerExpirations } from '../lib/dates';
...
//
// The component below fits SSVI jointly across several near-term expirations,
// overlays the SSVI curves on the observed smile for each tenor, and
// reports ρ, η, γ, per-slice RMSE, and the arb-free diagnostics. It is
// the shared implementation rendered from `discrete/slots/SlotF.jsx`
// (as the sixth slot on /discrete/) and from `src/App.jsx` (as the
// SSVI surface panel near the bottom of the landing page).
// -----------------------------------------------------------------------------
...
// Inline placeholder styles. The component is rendered on both /discrete/
// (which imports page.css and so could have used the .page-placeholder
// class) and on the landing page (which does not import page.css). Inlining
// the styles makes the component standalone — it renders identically on
// any surface that has theme.css available, without needing each consumer
// surface to opt into page.css just for the loading-state chrome.
...
export default function SsviSurfaceFit({ compact = false } = {}) {
  ...
  useEffect(() => {
    ...
    ssvi.slicePerf.forEach((slice, idx) => {
      ...
      // Observed IVs (markers). The legend label here is the only legend
      // entry for this tenor (the SSVI line below carries `showlegend:
      // false` and shares a legendgroup so clicking either toggles both).
      // The label stops at expiry · DTE — the trailing "· obs" segment
      // was dropped so each entry fits in one row of the horizontal
      // legend at typical card widths rather than wrapping to two rows.
      traces.push({
        x: slice.samples.map((s) => s.k),
        y: slice.samples.map((s) => s.iv * 100),
        mode: 'markers',
        name: `${slice.expirationDate} · ${dteLabel}`,
    ...
    const layout = plotly2DChartLayout({
      ...
      legend: {
        ...
        // below the "Log-Moneyness k = ln(K/F)" axis title. With the
        // trailing "· obs" segment dropped from each entry, the
        // horizontal legend now fits in one row at typical card
        // widths; the extra bottom margin stays so a narrower viewport
        // that still wraps the legend has room.
        y: -0.34,
        x: 0.5,
        xanchor: 'center',
        yanchor: 'top',
  ...
  if (loading && !data) {
    return (
      <div style={PLACEHOLDER_STYLE}>
        <div style={PLACEHOLDER_TITLE_STYLE}>Loading chain…</div>
        <div style={PLACEHOLDER_HINT_STYLE}>
  ...
  if (error) {
    return (
      <div style={{ ...PLACEHOLDER_STYLE, borderColor: 'var(--accent-coral)' }}>
        <div style={{ ...PLACEHOLDER_TITLE_STYLE, color: 'var(--accent-coral)' }}>
          Chain fetch failed
        </div>
        <div style={PLACEHOLDER_HINT_STYLE}>{error}</div>
      </div>
    );
  }
  if (plotlyError) {
    return (
      <div style={{ ...PLACEHOLDER_STYLE, borderColor: 'var(--accent-coral)' }}>
        ...
        <div style={PLACEHOLDER_HINT_STYLE}>{plotlyError}</div>
      </div>
    );
  }
